...
void deeplabv2(char *type, char *path)
{
    ...
    // pool4
    layers[14] = make_convolutional_layer(512, 3, 1, 4, 4, 1, "relu");
    layers[15] = make_convolutional_layer(512, 3, 1, 4, 4, 1, "relu");
    layers[16] = make_convolutional_layer(512, 3, 1, 4, 4, 1, "relu");
    layers[17] = make_maxpool_layer(2, 2, 0);
    // fc6
    layers[18] = make_convolutional_layer(1024, 1, 1, 0, 0, 1, "relu");
    // fc7
    layers[19] = make_convolutional_layer(1024, 1, 1, 0, 0, 1, "relu");
    //ASPP模块
    layers[20] = make_convolutional_layer(num_class, 3, 1, 6, 6, 1, "linear");
    layers[21] = make_shortcut_layer(layers[20], 1, "linear");
    layers[22] = make_convolutional_layer(num_class, 3, 1, 12, 12, 1, "linear");
    layers[23] = make_shortcut_layer(layers[20], 1, "linear");
    layers[24] = make_convolutional_layer(num_class, 3, 1, 18, 18, 1, "linear");
    layers[25] = make_shortcut_layer(layers[20], 1, "linear");
    layers[26] = make_convolutional_layer(num_class, 3, 1, 24, 24, 1, "linear");
    layers[27] = make_shortcut_layer(layers[21], 0, "linear");
    layers[28] = make_shortcut_layer(layers[23], 0, "linear");
    layers[29] = make_shortcut_layer(layers[25], 0, "linear");
    layers[30] = make_interpolate_layer(320, 320);
    layers[31] = make_crossentropy_layer(NULL, -1);
