...
void im2col(float *img, int height, int width, int channel, int ksize, int stride, int pad, int dilation, float *space)
{
    ...
    for (int c = 0; c < channels_col; ++c)
    {
        ...
        for (int h = 0; h < height_col; ++h)
        {
            ...
            {
                int im_row = h_offset + h * stride - pad + h_offset * dilation;
                int im_col = w_offset + w * stride - pad + w_offset * dilation;
...
void col2im(float *img, int ksize, int stride, int pad, int dilation, int out_h, int out_w, int out_c, float *space)
{
    int dksize = ksize*(dilation+1)-dilation;
    ...
    for (int i = 0; i < ksize*ksize*out_c; ++i){
        for (int j = 0; j < height_col*width_col; ++j){
            ...
            int index_h = (i-channel*ksize*ksize)/ksize + kernel_h - pad + kernel_h * dilation;
            int index_w = (i-channel*ksize*ksize)%ksize + kernel_w - pad + kernel_w * dilation;
