...
Layer *make_logsoftmax_layer(int group)
{
    ...
    l->initialize = init_logsoftmax_layer;
    l->forward = forward_logsoftmax_layer;
    l->backward = backward_logsoftmax_layer;

    l->initializegpu = init_logsoftmax_layer_gpu;
    l->forwardgpu = forward_logsoftmax_layer_gpu;
    l->backwardgpu = backward_logsoftmax_layer_gpu;
...
void forward_logsoftmax_layer(Layer l, int num)
{
    for (int i = 0; i < num; ++i){
        ...
        log_softmax(input, l.inputs, output);
    }
}
...
void backward_logsoftmax_layer(Layer l, int num, float *n_delta)
{
    for (int i = 0; i < num; ++i){
        ...
        log_softmax_gradient(input, l.inputs, delta_l);
